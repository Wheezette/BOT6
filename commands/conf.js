...
class Conf extends Command {
  ...
  // Retrieve Default Values from the default settings in the bot.
    ...
    if (action === "add") {
      if (!key) return message.reply("Proszę, abyś podał(a) klucz, który chcesz dodać.");
      if (defaults[key]) return message.reply("Podany przez Ciebie klucz już istnieje!");
      if (value.length < 1) return message.reply("Podaj wartość, jaką ma mieć dodany klucz.");
      ...
      message.reply(`Klucz **${key}** został pomyślnie dodany z wartością **${value.join(" ")}**`);
    } else
  
    // Changing the default value of a key only modified it for guilds that did not change it to another value.
    if (action === "edit") {
      if (!key) return message.reply("Proszę, abyś podał(a) klucz, który chcesz dodać.");
      if (!defaults[key]) return message.reply("Ten klucz nie istnieje w tych ustawieniach.");
      if (value.length < 1) return message.reply("Podaj wartość, na którą chcesz zmienić.");

      defaults[key] = value.join(" ");

      this.client.settings.set("default", defaults);
      message.reply(`Klucz **${key}** został edytowany na **${value.join(" ")}**`);
    } else
